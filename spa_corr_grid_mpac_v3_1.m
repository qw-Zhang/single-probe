...
function spatial_output = spa_corr_grid_simulation_mpac_v3_1(phi_sample,phi_a,d,error_para,ant_able)
    fc = 2.45e9;
    c = 3e8;
    lambda = c/fc;
...
%     phi_a = 0*pi/180;
    ...
    scenario = 'single';
    [ideal_PAS,PAS_para] = generate_PAS1(ideal_phi,scenario);
    
    % real probe scenario
    [real_PAS,phi_sample] = PAS_sample(ideal_phi,ideal_PAS,PAS_para,phi_sample,rate,'narrow');
...
%             h_sig_real_1(i,j) = alpha*exp(1j*(beta(j) + 2*pi*fc*(dot(pos_ant_1,top)/c) )).*sqrt(real_PAS(j));
...
%         spatial_circle_real_sig_MPAC(i) = sum(rx_real_1(i,:,num).*conj(rx_real_2(i,:,num)));
    ...
    for i = 1:50
    	ss(i) = mean(rx_real_2_sum(50-i+1,:).*conj(rx_real_2_sum(50+i,:)));
        num = randi([1,length(sig)],1);
        %randomly choose one point in signal sequence. "100" is chosen randomly
