...
% date sample rate: 20Mbps 
% delta_t / delay_resolution = 74000
data = randi([0 1],148000,1); 
txSig = qpskModulator(data);
txSig_inter = resample(txSig,10,1);
rxSig1_interp = zeros(T,size(txSig_inter,1) + TAU - 1);
rxSig2_interp = zeros(T,size(txSig_inter,1) + TAU - 1);
rxSig1_temp = zeros(T,size(txSig,1));
rxSig2_temp = zeros(T,size(txSig,1));
rxSig1_res = zeros(size(rxSig1_temp,1) * size(rxSig1_temp,2),1);
rxSig2_res = zeros(size(rxSig1_temp,1) * size(rxSig1_temp,2),1);
