...
% v2.2 delete the fading channel h instead of a time unvariant coffeicent
function spatial_output = spa_corr_grid_mpac_v2_2(phi_sample,phi_a,d,error_para,ant_able)
fc = 2.45e9;
c = 3e8;
...
%%
% generate h (channel parameters) (old version)
...
%%
faded = 1;
[h_syn,h_para] = generate_H(phi_sample,real_PAS,scenario,faded);
...
for i = 1:length(new_d)
    ...
    for j = 1:length(phi_sample)
        ...
        fading_sig = generate_fading_sig(h_syn,h_para,j,sig);
        h1 = squeeze(sum(h_syn.h1,1));
        h2 = squeeze(sum(h_syn.h1,1));
        len_h = size(h1,2);
        h_probe_1(j,:) = h1(j,:) .* exp(1j*k_CONST*(d_real_1(i,j)) );
        h_probe_2(j,:) = h2(j,:) .* exp(1j*k_CONST*(d_real_2(i,j)) );
        ...
        for t_index = 1:T
        rx_h_sig_1 = fading_sig(t_index,:) .* exp(1j*k_CONST*(d_real_1(i,j)) );
        rx_h_sig_2 = fading_sig(t_index,:) .* exp(1j*k_CONST*(d_real_2(i,j)) );
        rx_h_sig_fft_1(t_index,i,j) = fft_process( rx_h_sig_1 );
        rx_h_sig_fft_2(t_index,i,j) = fft_process( rx_h_sig_2 );
        end
    ...
    sig_sum_1 = sum(rx_h_sig_fft_1,3);
    sig_sum_2 = sum(rx_h_sig_fft_2,3);
    ...
    %         for t_index = 1:size(h_probe_1,2)
    %         temp_h(t_index) = sum(h_probe_1(:,t_index) .* conj(h_probe_2(:,t_index)));
    %         end
    %         ss_1(i) = mean(temp_h);
